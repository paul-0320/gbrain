...
//
// bump 18→19 (fork-carry, ymyd): `kof=` (keyword AND→OR fallback knob) joins
// the key. Upstream #3617 is still open; upstream took 16 (#3515 det=),
// 17 (WP2/T3 degradation stamp) and 18 (#1863 acm=) meanwhile — this fork
// renumbers rather than collide. Same one-time global cold-miss pattern as
// the bumps above.
...
export function knobsHash(
  ...
  const parts = [
    ...
    // v=19 addition (append-only, fork-carry): keyword AND→OR fallback knob. A
