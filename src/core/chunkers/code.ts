...
export async function chunkCodeTextFull(
  ...
  try {
    ...
    // Fork: layer both caps. Upstream's capOversizedChunks (#1675) splits by
    // the generic estimateTokens (~3.5 chars/token), which undercounts CJK
    // (~1 char/token) and URL-dense text — a Korean chunk 3x over the real
    // limit passes its trigger untouched. capCodeChunks re-checks with the
    // CJK-aware estimateEmbeddingTokens and is a no-op when all under cap.
