...
describe('knobsHash determinism + cross-mode separation (CDX-4)', () => {
  ...
  test('KNOBS_HASH_VERSION constant exposed for migrations to bump on schema change', () => {
    ...
    // 26→27: qi= (query-side instruct template, fork carry) joins the key.
...
describe('v0.42.3.0 — autocut knobs', () => {
  test('KNOBS_HASH_VERSION is 27 (21→22 result-stamp/injection epoch #1663 #3995 #3783 #4220; 22→23 excludePrivate posture fold #4352; 23→24 negative-offset cache-skip gap #4358 residual; 24→25 keywordOrFallback knob kof=; 25→26 salience/recency + intent_patterns fold #4415; 26→27 query-instruct qi= fork carry)', () => {
    // 26→27: qi= (query-side instruct template, fork carry) joins the key.
    expect(KNOBS_HASH_VERSION).toBe(27);
  });
